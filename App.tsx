...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // 1. Fetch Movies from Firestore - Real data only, no demo fallback
    const q = query(collection(db, 'movies'), orderBy('createdAt', 'desc'), limit(100));
    ...
    const unsubscribeMovies = onSnapshot(q, (snapshot) => {
      ...
      // ✅ Duplicate fix: id দিয়ে unique করা (same id দুইবার আসলে একটাই রাখবে)
      ...
      // ✅ Demo data নেই - Firestore empty হলে empty array
      setMovies(uniqueMovies);
    }, (error) => {
      console.warn("Firestore access failed:", error);
      setMovies([]); // empty - demo নয়
    });
  ...
  return (
    ...
    >
      ...
      <main className={`px-4 max-w-7xl mx-auto relative z-10 ${activeTab === 'home' ? '-mt-10' : 'pt-20'}`}>
        ...
        {activeTab === 'home' && (
            <>  
                {/* 1. Continue Watching - NEW SECTION */}
                ...
                {/* 2. Trending - isTop10 flag দিয়ে ফিল্টার করা */}
